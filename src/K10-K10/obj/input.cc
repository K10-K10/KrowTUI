...
TextField& TextField::move_cursor(int x, int y) {
  ...
  if (y >= contents_.size()) return *this;
  ...
  if (x > text_glyph_count) return *this;
...
TextField& TextField::insert_char(const std::string& c) {
  if (cursor_y >= contents_.size()) return *this;
  ...
  for (const auto& g : glyphs) merged_str += g;
  Line new_line = Line();
  Span s = merged_str;
  s.style(text_style_);
...
void TextField::draw() {
  ...
  for (int screen_y = 0; screen_y < h; ++screen_y) {
    ...
    // 背景をデフォルトスタイルでまず一行クリア
    for (int sx = 0; sx < w; ++sx) {
      __krow__::drawObj.put(cy, l + sx, {" ", text_style_});
    }

    if (data_y >= contents_.size()) continue;
